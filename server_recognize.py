        ...
        for i in range(len(bboxs)):
            ...
            if name == null:
                ...
                if score < 0.6:
                    ...
                    cv2.imwrite('./dataset/unknown/'+caption+'%d.jpg'%(unknown_cnt), face_image) # Capture face image
                    if unknown_cnt > 300 :
                        for i in range(1, 300, 5) :
                            remove('./dataset/unknown/'+caption+'%d.jpg'%(i))
                        unknown_cnt=0
                        
                else:
                    caption = f"{name.split('_')[0].upper()}:{score:.2f}"

                #t_size = cv2.getTextSize(caption, cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, 2)[0]
                #cv2.rectangle(frame, (x1, y1), (x1 + t_size[0], y1 + t_size[1]), (204, 204, 255), -1)
                #cv2.putText(frame, caption, (x1, y1 + t_size[1]), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, [0, 0, 0], 2)       
